
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/User'
export type * from './models/Agent'
export type * from './models/FollowUpCampaign'
export type * from './models/FollowUpMessage'
export type * from './models/BroadcastCampaign'
export type * from './models/BroadcastRecipient'
export type * from './models/OrchestratorAgent'
export type * from './models/Document'
export type * from './models/DocumentChunk'
export type * from './models/MediaItem'
export type * from './models/Conversation'
export type * from './models/Message'
export type * from './models/ConversationSummary'
export type * from './models/MessageTemplate'
export type * from './models/Lead'
export type * from './models/ConversationRead'
export type * from './models/Customer'
export type * from './models/Referral'
export type * from './models/NewsletterSubscriber'
export type * from './models/ConversationLog'
export type * from './models/WorkspaceMember'
export type * from './models/PaymentRequest'
export type * from './models/BaileysSession'
export type * from './models/BaileysOutboundLog'
export type * from './models/CreditUsage'
export type * from './commonInputTypes'