...
describe("apiBilling — chargeApiTurn / remaining (real DB)", () => {
  ...
  const ctx = (): AgentBillingContext => ({ userId, plan: "free", subscriptionExpiresAt: null })
  ...
  it("preflight blocks an expired subscription", async () => {
    ...
    const res = await preflightApiCharge({ userId, plan: "free", subscriptionExpiresAt: past }, agentId)
