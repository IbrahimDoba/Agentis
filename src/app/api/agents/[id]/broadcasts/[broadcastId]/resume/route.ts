...
export async function POST(
  ...
  try {
    ...
    await baileysClient.resumeBroadcast(broadcastId)
    return NextResponse.json({ success: true })
  } catch (error) {
    console.error("[POST /api/agents/:id/broadcasts/:broadcastId/resume]", error)
    return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to resume broadcast" }, { status: 502 })
  }
}
